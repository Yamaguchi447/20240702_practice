...
def is_pareto_efficient(costs, minimize, return_mask=True):
    ...
    :param minimize: A list of booleans indicating whether each objective should be minimized
    :param return_mask: True to return a mask, False to return indices
    :return: An array of indices or a boolean mask
    """
    # Adjust costs according to minimize flag
    adjusted_costs = costs.copy()
    for i, min_flag in enumerate(minimize):
        if not min_flag:
            adjusted_costs[:, i] = -adjusted_costs[:, i]
    
    is_efficient = np.ones(adjusted_costs.shape[0], dtype=bool)
    for i, c in enumerate(adjusted_costs):
        if is_efficient[i]:
            is_efficient[is_efficient] = np.any(adjusted_costs[is_efficient] <= c, axis=1)
            is_efficient[i] = True
...
# パレートフロントの抽出
costs = data[["val_accuracy", "time"]].to_numpy()
pareto_mask = is_pareto_efficient(costs, minimize)
...
ax.scatter(data["val_accuracy"], data["time"], c='blue', label='All solutions')
ax.scatter(pareto_data["val_accuracy"], pareto_data["time"], c='red', label='Pareto front')
